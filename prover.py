"""
3806ICT Assignment 1 starter implementation.

Purpose:
- Parse first-order logic formulae from text.
- Represent formulae and terms as Python data classes.
- Implement the baseline backward proof-search strategy based on Algorithm 2.
- Leave clear extension points for the improved version and experiments.

Important:
- This is a learning scaffold. You should test, modify, and explain your own design choices.
- The assignment report must be written by you, not generated by AI.
"""

from __future__ import annotations

from dataclasses import dataclass, field
from typing import FrozenSet, Iterable, List, Optional, Sequence, Tuple, Union, Dict, Set
...
@dataclass
class SearchResult:
    status: str                  # "proved", "failed", or "timeout"
    elapsed: float
    nodes_expanded: int
    root: ProofNode
...
class BaselineProver:
    ...
    def _expand(self, node: ProofNode) -> bool:
        ...
        # A branch is proved only if every child branch closes.
        all_closed = True
    ...
    def _closing_rule(self, seq: Sequent) -> Optional[str]:
        # id: same formula appears on both sides.
    ...
    def _next_baseline_step(self, seq: Sequent) -> Optional[Tuple[str, List[Sequent]]]:
        # Non-branching propositional and eigenvariable rules first.
        ...
        # Quantifier rules that keep their main formula: ∀L and ∃R.
...
class ImprovedProver(BaselineProver):
    """
    Improved version for the assignment comparison.

    Improvements:
    1. Memoises sequents that have already been proved or failed.
    2. Detects repeated sequents on the current recursive path to reduce loops.
    3. Avoids duplicate quantifier instantiations that do not change the sequent.
    4. Tries all currently available ground terms for ∀L and ∃R before giving up.

    The baseline keeps Algorithm 2's naïve behaviour. This improved version changes
    only the search strategy; it is not a wrapper around an external solver.
    """

    def __init__(self, max_nodes: int = 10_000, max_seconds: float = 5.0):
        super().__init__(max_nodes=max_nodes, max_seconds=max_seconds)
        self.memo: Dict[Sequent, bool] = {}
    ...
    def _expand(self, node: ProofNode) -> bool:
        ...
        if seq in self.memo:
            node.rule = "memo-hit"
            node.closed = self.memo[seq]
            return self.memo[seq]
        ...
        self.memo[seq] = result
        return result

    def _next_baseline_step(self, seq: Sequent) -> Optional[Tuple[str, List[Sequent]]]:
        # Keep the same propositional and eigenvariable rule order as the baseline.
        for f in sorted(seq.left, key=str):
            if isinstance(f, And):
                return "∧L", [replace_left(seq, f, [f.left, f.right])]
            if isinstance(f, Not):
        ...
        # Improved quantifier policy: only create useful instances.
        ground_terms = self._available_ground_terms(seq)
        ...
        # Fresh-term fallback, but only if it adds a genuinely new instance.
        for f in sorted(seq.left, key=str):
            if isinstance(f, Forall):
                c = self.fresh_const()
...
class Parser:
    ...
    def parse_term(self) -> Term:
        ...
        # Simple convention: x, y, z are variables; other lowercase names are constants.
        # Bound variables are handled by formula substitution later.
...
EXAMPLES = [
    ...
    "(P(a) && Q(a)) -> P(a)",
    "P(a) -> (P(a) || Q(a))",
    # Parentheses matter: without them, this parses as forall x. (P(x) -> P(a)).
    "(forall x. P(x)) -> P(a)",
